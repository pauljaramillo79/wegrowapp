...
router.post("/loadcurrentbudget", (req, res) => {
  ...
  db.query(
    ...
    "SELECT YEAR(budgets.date) AS yy, QUARTER(budgets.date) AS qq, countryList.country, prodCatNames.prodCatName, prodNames.abbreviation, budgets.quantity AS budget, IFNULL(quarterlysales.quantity,0) AS sold FROM (budgets INNER JOIN countryList ON budgets.countryID = countryList.countryID INNER JOIN prodNames ON budgets.prodNameID = prodNames.prodNameID INNER JOIN prodCatNames ON budgets.prodCatNameID = prodCatNames.prodCatNameID) LEFT JOIN quarterlysales ON CONCAT(DATE_FORMAT(date,'%Y'), QUARTER(date),budgets.prodNameID, budgets.countryID) = CONCAT(y,Q,productName, quarterlysales.countryID) WHERE YEAR(budgets.date) = ? UNION SELECT y AS yy, Q AS qq, country, prodCatName, abbreviation, 0, quantity AS sold from quarterlysales WHERE y = ?;",
