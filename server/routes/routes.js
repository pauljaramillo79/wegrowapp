...
router.post("/loadcurrentbudget", (req, res) => {
  ...
  db.query(
    ...
    "SELECT YEAR(budgets.date) AS yy, QUARTER(budgets.date) AS qq, countryList.country, countryList.wgregions AS region, prodCatNames.prodCatName, prodNames.abbreviation, budgets.quantity AS budget, IFNULL(quarterlysales.quantity,0) AS sold, budgets.profit*budgets.quantity AS budgetprofit, IFNULL(quarterlysales.profit,0) AS soldprofit FROM (budgets INNER JOIN countryList ON budgets.countryID = countryList.countryID INNER JOIN prodNames ON budgets.prodNameID = prodNames.prodNameID INNER JOIN prodCatNames ON budgets.prodCatNameID = prodCatNames.prodCatNameID) LEFT JOIN quarterlysales ON CONCAT(DATE_FORMAT(date,'%Y'), QUARTER(date),budgets.prodNameID, budgets.countryID) = CONCAT(y,Q,productName, quarterlysales.countryID) WHERE YEAR(budgets.date) = ? UNION SELECT y AS yy, Q AS qq, country, region, prodCatName, abbreviation, IFNULL(budgets.quantity,0), quarterlysales.quantity AS sold, IFNULL(budgets.profit*budgets.quantity,0), IFNULL(quarterlysales.profit,0) AS soldprofit from quarterlysales LEFT JOIN budgets ON CONCAT(DATE_FORMAT(date,'%Y'), QUARTER(date),budgets.prodNameID, budgets.countryID) = CONCAT(y,Q,productName, quarterlysales.countryID) WHERE y = ?;",
